...
namespace ZLibNet
{
	...
	public class Zipper
	{
        ...
        // event handling
        public event Action<string> eventExeptionWarning;
        public event Action<string, int, int> eventFileAdding;    // filename, current num, total num
        public event Action<string, int, int> eventFileSkipping;  // filename, current num, total num
        public event Action<int> eventFilePercent;
			...
			try
			{
				...
				using (ZipWriter writer = new ZipWriter(ZipFile))
				{
					...
					foreach (FileSystemEntry fsEntry in fsEntries)
					{
                        ...
                        if (isInterrupting) throw new Exception("Interrupt request recived !");

                        if (IsIncludeFile(fsEntry.ZippedName, fsEntry.IsDirectory, includes, excludes))
                        {
                            if (eventFileAdding != null) eventFileAdding(fsEntry.FullName, fsEntries.IndexOf(fsEntry), fsEntries.Count);
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    using (FileStream reader = File.Open(fi.FullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                                    {
                                        int byteCount;
                                        int percent = 0;
                                        long processed = 0;
                                        ...
                                        {
                                            while (isPaused) Thread.Sleep(250);
                                            if (isInterrupting) break;
                                            writer.Write(pBuffer, 0, byteCount);
                                            processed += byteCount;
                                            int newPercent = (int)(processed * 100 / fi.Length);
                                            if (percent != newPercent)
                                            {
                                                percent = newPercent;
                                                if (eventFilePercent != null) eventFilePercent(percent);
                                            }
                                        }
                                    }
                                }
                                ...
                                {
                                    if (eventExeptionWarning != null) eventExeptionWarning("'" + fi.Name + "' - " + ex.Message.Trim('\r', '\n'));
                                }
                            }
                            addedSomeEntry = true;
                        }
                        ...
                        {
                            if (eventFileSkipping != null) eventFileSkipping(fsEntry.FullName, fsEntries.IndexOf(fsEntry), fsEntries.Count);
                        }
					}
				}
